...
description: Use when the user asks to summarize the session, generate a session report, or requests a session summary HTML document. Produces a branded {ORG_NAME} session summary.
version: 1.0.0
---

# Session Summary

Generate a branded, standalone HTML document summarizing the current session's accomplishments. Output follows {ORG_NAME} brand guidelines.

## Usage
```
/{ORG_ID}:session-summary                           # Auto-generate title from session context
/{ORG_ID}:session-summary Dashboard UX Overhaul     # Explicit title
```
...
### Step 1b: Detect Project Branding

Before generating HTML, check for project-specific branding:

1. Look for `Agents/Brand/Memory/BRAND.md` in the current project directory
2. If found, scan the `## Colors` section for lines matching `<!-- Primary: #hex -->` and `<!-- Accent: #hex -->`
3. Also read the project name from `.claude-project` if present
4. If color values are found:
   - Use primary color to override `--jr-teal` in the HTML template CSS
   - Use accent color to override `--jr-green`
   - Use project/client name in the header subtitle and footer instead of "{ORG_NAME}"
5. If BRAND.md is missing or color values are not populated: use {ORG_NAME} default colors

### Step 2: Determine Filename

Format: `{Agent}_{yyyyMMdd}_{hhmmtt}_{TZ}_{BriefTitle}.html`

Examples:
- `Operator_20260323_0415pm_CST_DashboardUXOverhaul.html`
- `Operator_20260403_0900am_CDT_AgentTeamScaffolding.html`

**Get the actual current time with a Bash tool call** — do not infer or guess:
```bash
date +"%Y%m%d_%I%M%p_%Z"
```
This produces `20260323_0415PM_CST`. Lowercase the `AM`/`PM` portion for the filename.

Title should be PascalCase, no spaces, max ~30 chars.
...
- **{ORG_NAME} branded** — Montserrat font, brand color palette, teal gradient header
...
Use this exact structure. Fill in sections based on session context. Omit sections that don't apply.
...
<head>
    ...
    <style>
        :root {
            ...
            --jr-teal: #0a4d68;
            --jr-dark-gray: #4b4b4b;
            --jr-dark-green: #1e6b48;
            --jr-light-gray: #e6e6e6;
            --jr-pale-green: #e1f0e8;
            --jr-sage: #d7e8dc;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Montserrat', Arial, sans-serif; color: var(--jr-dark-gray); line-height: 1.7; background: #f8f9fa; }
        header { background: linear-gradient(135deg, var(--jr-teal) 0%, var(--jr-dark-green) 100%); color: white; padding: 2.5rem 2rem; }
        ...
        h2 { color: var(--jr-teal); font-size: 1.5rem; font-weight: 600; margin: 2rem 0 0.75rem; padding-bottom: 0.4rem; border-bottom: 3px solid var(--jr-green); }
        h3 { color: var(--jr-teal); font-size: 1.1rem; font-weight: 500; margin: 1.25rem 0 0.5rem; }
        p, li { margin-bottom: 0.5rem; font-size: 0.95rem; }
        ul { padding-left: 1.25rem; }

        /* Stats bar */
        .stats { display: flex; gap: 1rem; flex-wrap: wrap; margin: 1.5rem 0; }
        .stat { background: white; border: 1px solid var(--jr-light-gray); border-radius: 10px; padding: 1rem 1.25rem; text-align: center; flex: 1; min-width: 120px; }
        .stat .number { font-size: 1.75rem; font-weight: 700; color: var(--jr-teal); }
        .stat .label { font-size: 0.75rem; color: var(--jr-gray); text-transform: uppercase; letter-spacing: 0.05em; margin-top: 0.15rem; }

        /* Callouts */
        .callout { border-left: 4px solid; padding: 0.75rem 1rem; margin: 1rem 0; border-radius: 0 6px 6px 0; font-size: 0.9rem; }
        .callout-green { border-color: var(--jr-green); background: #f0f7e6; }
        .callout-gold { border-color: #ddb027; background: #fdf8e8; }
        .callout-orange { border-color: #a95b28; background: #fdf0e8; }
        .callout-teal { border-color: var(--jr-teal); background: #e8f4f5; }
        .callout strong { font-weight: 600; }

        /* Tables */
        table { width: 100%; border-collapse: collapse; margin: 0.75rem 0 1.25rem; font-size: 0.9rem; }
        thead { background: var(--jr-pale-green); }
        th { text-align: left; padding: 0.6rem 0.75rem; font-size: 0.8rem; font-weight: 600; color: var(--jr-teal); border-bottom: 2px solid var(--jr-green); text-transform: uppercase; letter-spacing: 0.03em; }
        td { padding: 0.6rem 0.75rem; border-bottom: 1px solid var(--jr-light-gray); }
        tr:hover { background: var(--jr-sage); }
        ...
        /* File lists */
        .file-path { font-family: 'Courier New', monospace; font-size: 0.85rem; color: var(--jr-teal); }

        /* Footer */
        footer { text-align: center; padding: 1.5rem; color: var(--jr-gray); font-size: 0.8rem; border-top: 1px solid var(--jr-light-gray); margin-top: 2rem; }
...
<body>
    ...
    <div class="container">
        ...
        <!-- WHAT WAS ACCOMPLISHED — always include -->
    ...
    <footer>
        <strong>{ORG_NAME}</strong> — {{PROJECT_NAME}}<br>
...
- Use callout boxes for important wins

### Blockers Resolved
- Table format: Issue | Root Cause | Fix
- Only include real blockers that were diagnosed and fixed
